...
void stock_screen_ui(void)
{
    ...
    ESP_LOGD(TAG, "Create 'timer_update_weather_box' timer with %dms period", PAGE_TIMER_UPDATE_TICK);
    timer_stock_data = lv_timer_create(_updateStockData, PAGE_TIMER_UPDATE_TICK, NULL);
    ...
    if (stock_portfolio != NULL)
    {
        if (!first_updated || update_tick > 120)
        {
            ...
            for (uint8_t ind = 0; ind < stk_box_cnt; ind++)
            {
                ...
                {
                    sprintf(stock_name, "--");
                    sprintf(stock_price, "A$ --.--");
                    sprintf(stock_change, "-.-- %%");
                }
                ...
                for (uint8_t lbl_ind = 0; lbl_ind < lbl_cnt; lbl_ind++)
                {
                    ...
                    case 2:
                        ...
                        {
                            lv_obj_set_style_text_color(box_lbl, RED_STOCK, 0);
                        }
                        else
                        {
                            lv_obj_set_style_text_color(box_lbl, BLUE_STOCK, 0);
                        }
                        break;
...
void _createStockBoxes(void)
{
    ...
    lv_style_set_text_color(&box_stock_txt_style, black_color);
